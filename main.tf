...
resource "aws_lb_listener_rule" "http_tcp_listener_rule" {
  count = local.create_lb || local.create_listener_rule ? length(var.http_tcp_listener_rules) : 0

  listener_arn = local.create_lb ? aws_lb_listener.frontend_http_tcp[lookup(var.http_tcp_listener_rules[count.index], "http_tcp_listener_index", count.index)].arn : var.http_tcp_listener_rules[count.index].value["http_tcp_listener_arns"]
  ...
  dynamic "action" {
    ...
    content {
      type             = action.value["type"]
      target_group_arn = local.create_lb ? aws_lb_target_group.main[lookup(action.value, "target_group_index", count.index)].id : var.http_tcp_listener_rules[count.index].action.value["target_group_arn"]
      }
  }
